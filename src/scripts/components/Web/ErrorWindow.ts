...
export default class ErrorWindow {
  ...
  private create(): void {
    ...
    this.scene.add.sprite(bg.x - 1, bg.getTopCenter().y + 14, 'header-syst').setOrigin(0.5, 1);
    this.scene.add.sprite(bg.x - 1, bg.getBottomCenter().y - 1, 'bottom-syst').setOrigin(0.5, 0);

...
export default class FarmWaterWindow {
  ...
  private create(): void {
    ...
    if (this.scene.state[`user${farm}`].part >= settings.unlock_improve) {
      ...
      const button = this.scene.bigButton('blue', 'left', -30, improveText, improve);
    ...
    const button1 = this.scene.bigButton('red', 'center', 60, this.scene.state.lang.exchangeWaterBtn);

...
export default class BuyFarmTerritoryWindow {
  ...
  private create(): void {
    ...
    if (this.scene.state[`user${farm}`].part >= settings.unlock || this.scene.state.userUnicorn.maxLevelAnimal >= settings.unlock) {
      ...
      let button = this.scene.bigButton('yellow', 'left', 20, this.scene.state.lang.buyTerritory, right);
  ...
  private unicornFarmBtns(settings: IeventTerritoriesPrice): void {
    ...
    let button = this.scene.bigButton('green', 'left', -15, this.scene.state.lang.buyTerritory, right1);
    this.scene.clickModalBtn(button, (): void => { this.buyTerritoryAndClose() });

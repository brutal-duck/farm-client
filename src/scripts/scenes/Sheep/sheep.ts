...
function collectWool(sheep: any, manualСollect: boolean = false): void {
  ...
  if (sheep.type !== 0) {
    ...
    if (manualСollect) {
      ...
      if (this.state.userSheep.feedBoostTimer > 0) price *= this.feedBoostMultiplier; // если бустер комбикорм активен
    ...
    sheep.wool = 0;
    sheep.diamond++;
    this.game.scene.keys['SheepBars'].plusDiamondsAnimation(position);

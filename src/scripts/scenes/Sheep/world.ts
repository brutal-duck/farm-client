...
function world(): void {
  ...
  this.state.sheepTerritories.map((data: Iterritories) => {
    ...
    if (data.type === 5) {
      ...
      territory.repository = this.add.image(x, y, 'sheep-repository-' + data.improve)
    ...
    if (data.type === 6) {
      ...
      this.house = this.add.image(x, y, 'sheep-house-' + this.state.userSheep.part)
        .setOrigin(0.5, 1)
        .setDepth(territory.y);
    ...
    if (data.type === 7) {
      ...
      if (this.state.userSheep.part < 3) texture = 'cave-disable';

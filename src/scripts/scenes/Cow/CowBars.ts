...
class CowBars extends Phaser.Scene {
  ...
  public create(): void {
    ...
    this.cowPrice = this.add.text(82, this.height - 43, Utils.checkSale(this.state, `${this.state.farm.toUpperCase()}_PRICE`) ? halfPrice : price, {
      font: '28px Bip',
      color: '#925C28',
      align: 'center'
  ...
  public updateCowPrice(): void {
    ...
    this.cowPrice.setText(Utils.checkSale(this.state, `${this.state.farm.toUpperCase()}_PRICE`) ? halfPrice : price);
    let bounds = this.cowPrice.getBounds();
  ...
  private updateSale(): void {
    const visibility = Utils.checkSale(this.state, `${this.state.farm.toUpperCase()}_PRICE`) && this.cowBuy.visible;

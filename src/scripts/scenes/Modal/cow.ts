...
function cowProfile(): void {
  ...
  let farmer: Phaser.GameObjects.Text = this.add.text(305, 0, this.state.lang.cowProfileName + ' ' + romanize(this.state.userCow.part), {
    font: '24px Bip',
    color: '#925C28',
    align: 'left',
    wordWrap: { width: 310 }
...
function improveCollectorCow(): void {
  ...
  let speedText: string = this.state.lang.speed + ': ' + thisLevel.speed + ' ' + this.state.lang.unitCow + '/' + this.state.lang.seconds;
  ...
  if (this.state.userCow.part >= nextLevel.chapter) {
    ...
    this.clickModalBtn(improve, (): void => {
      ...
      this.updateImproveCollectorCow(improve, speed, duration, nextLevelText);  
...
function updateImproveCollectorCow(
  btn: {

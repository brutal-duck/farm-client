...
function updateMapNotification(): void {
  ...
  const count: number = this.state.farm === 'Sheep' && this.state.userSheep.tutorial >= 100 ? chickenCount + cowCount + unicornCount + socialCount + clanCount:
...
function checkFactoryCow(): boolean {
  if (this.state.farm === 'Cow') {
    ...
    return factory && factory.money > 0 && !factory.currentProduction;
  } 
